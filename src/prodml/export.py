...
def export_model():
    ...
    logger.info(f"Pickle - Mean: {np.mean(times_pkl)*1000:.2f}ms | p95: {np.percentile(times_pkl, 95)*1000:.2f}ms")
    logger.info(f"ONNX   - Mean: {np.mean(times_onnx)*1000:.2f}ms | p95: {np.percentile(times_onnx, 95)*1000:.2f}ms")
