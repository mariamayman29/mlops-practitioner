...
class duration_predictor:
    ...
    def load_model(self):
        ...
        if not self.is_loaded:
            logger.info(f"Loading model from {Config.onnx_model_path}")
            self.session = rt.InferenceSession(Config.onnx_model_path)
    ...
    def predict_trips(self, trips_info: list[dict]) -> list[float]:
        ...
        X = self.dv.transform(trips_info).toarray().astype(np.float32)
        input_name = self.session.get_inputs()[0].name
        predicted_durations = [
            float(duration.item())
            for duration in self.session.run(None, {input_name: X})[0]
        ]
        logger.info(f"Predicted durations for trips: {predicted_durations}")
